...
function downLoad(){
	   ...
	   function drawProgress(){
		  ...
		  //画环形圆
		  ctx.beginPath();
          ctx.arc(canvasWidth/2,canvasHeight/2-50,50,0, Math.PI * 2,true);
		  ctx.closePath();
		  ctx.fillStyle = '#000';  
	      ctx.fill(); 
		  ...
		  //画进度/扇形圆
		  ctx.beginPath();
          ctx.arc(canvasWidth/2,canvasHeight/2-50,50,0, Math.PI *2*progress/100, true);  
		  ctx.closePath();  
		  ctx.fillStyle = '#fff';  
		  ctx.fill();  
		  ...
		  //画内部空白
		  ctx.beginPath();  
		  ctx.arc(canvasWidth/2,canvasHeight/2-50,44, 0, Math.PI * 2, true);  
	      ctx.closePath();  
		  ...
		  //画一条线
		  ctx.beginPath();  
		  ctx.arc(canvasWidth/2,canvasHeight/2-50,41, 0, Math.PI * 2, true);  
	      ctx.closePath();  
		  ctx.fillStyle='#ddd';  
		  ctx.fill();
...
function Hero(imgs){
   ...
   this.move=function(){//移动飞机
	   ...
	   if(this.counter%3===0){
		...
		//边移动边发射子弹
		if(this.counter%5===0){//此处10指定每两发子弹的间隔，值越小，则发送的越快
		    this.fire();
		}  
   }
   //发射子弹 
...
function Bullet(img){
	...
	this.move=function(){
	   this.y-=8;//此处的5指定子弹移动的速度，可以设置为全局变量
